...
/**
 * The domain-value -> (label, tone) tables behind {@link StatusPill}.
 *
 * <p>Split out of `StatusPill.tsx` so the mapping can be consumed as data by
 * callers that need the wording without the badge around it (see
 * {@link statusLabel}). Keeping it in the component file made that a non-
 * component export, which breaks React Fast Refresh for the whole module.
 *
 * <p>Presentation-only mapping, not a business rule — the domain values
 * themselves come from the backend (ADR-006: frontend consumes read models).
 */
...
export const TABLES: Record<StatusKind, Record<string, StatusMapping>> = {
...
/**
 * The label half of the mapping, without the badge around it.
 *
 * <p>For the callers that need the same wording where a pill would be too loud
 * — the muscle legend beside the session detail's silhouettes prints "Carga
 * alta" as quiet 10px meta text, not as a bordered chip. Sharing the resolver
 * keeps that text and the pills elsewhere on the same screen from drifting into
 * two vocabularies for one domain value.
 *
 * <p>Same fallback as {@link StatusPill}: an unrecognized value returns itself,
 * so a label can never render blank.
 */
