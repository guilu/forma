...
import { groupMusclesForDisplay, muscleSummary } from './trainingMuscleLabels';
...
import { DAY_LABELS, KIND_LABELS, WEEK_ORDER } from './training/trainingLabels';
...
export function TrainingPage() {
  ...
  const { pendingId, mark, move } = useSessionActions({
    reload: load,
    // The dialog stays open on a status change and has to show the new one; a
    // move takes the session off the day the dialog was opened from, so it closes.
    ...
    onMoved: () => setDetailTarget(undefined),
  });
